...
const config = new Promise(async resolve => {
  ...
  const screenshots = answers.screenshots === 'yes'
      , params = answers.params === 'yes'
      , hash = answers.hash === 'yes'
      , build = path.join(path.resolve(''), answers.build)
...
const createGeneratamine = async () => {
  const configPath = path.join(__dirname, '.generataminerc')
  ...
  const { isConfig } = await inquirer.prompt({
    type: 'list',
    name: 'isConfig',
    message: 'Do you want to generate a configuration file .generataminerc ?',
    choices: [
      'yes',
      'no'
    ]
...
const createPage = async (browser, link) => {
  const { expectation, screenshots, build, hash, params } = (await config)()
  ...
  const dirPath = path.join(build, pathname)
  ...
  const links = await page.evaluate(({ hash, params }) =>
...
const createRobots = async () => {
  const { build, site } = (await config)()
  const pathfile = path.join(build, 'robots.txt')
...
const createSitemap = async () => {
  const { build, site, url } = (await config)()
  const pathfile = path.join(build, 'sitemap.txt')
...
const server = () => new Promise(
  async resolve => {
    const { launch, build } = (await config)()
    const app = express()
...
;(async () => {
  const { debugLaunch, url, build } = (await config)()
  ...
  console.log(`1.  yarn global add serve`)
  console.log(`2.  serve build`)
